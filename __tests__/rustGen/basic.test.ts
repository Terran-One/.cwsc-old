...
describe("ast compiler", () => {
    ...
    it("compiles a contract with an execute message", () => {
        ...
        const parser = Parser.fromString(code);
        const ast = parser.buildAST(); // ToDo: build AST manually instead of doing this
        ...
        const useEntryPoint = contract.items.find(x => (x as any).path === 'cosmwasm_std::entry_point') as any;
        expect(useEntryPoint).toBeDefined();
        expect(useEntryPoint.annotations).toContainEqual({ value: `cfg(not(feature = \"library\"))` });

        const useToBinaryAndOtherStuff = contract.items.find(x => (x as any).path === 'cosmwasm_std::{to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult}') as any;
        expect(useToBinaryAndOtherStuff).toBeDefined();
        expect(useToBinaryAndOtherStuff.annotations).toHaveLength(0);

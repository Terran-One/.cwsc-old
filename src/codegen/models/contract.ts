...
export class ContractCodegen extends ASTCodegen<AST.ContractDefn> {
  ...
  constructor(ast: AST.ContractDefn, env?: CWScriptEnv) {
    ...
    let model: any = {};
    model.name = ast.name.text;
    model.events = ast.descendantsOfType(AST.EventDefn);
    model.errors = ast.descendantsOfType(AST.ErrorDefn);
    model.state = [
      ...ast.descendantsOfType(AST.ItemDefn),
      ...ast.descendantsOfType(AST.MapDefn),
    ];
    model.instantiate = ast.descendantsOfType(AST.InstantiateDefn)[0];
    model.exec = ast.descendantsOfType(AST.ExecDefn);
    model.query = ast.descendantsOfType(AST.QueryDefn);
    model.fns = ast.descendantsOfType(AST.FnDefn);
    model.structs = ast.descendantsOfType(AST.StructDefn);

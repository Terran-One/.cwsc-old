import { ContractDefn, Ident, InstantiateDefn, List, SourceFile } from '../../ast/nodes';
import { CWScriptCodegen } from '../../codegen/codegen';
import { CodeGroup, Defn, Expr, FunctionArg, MatchPattern, Rust, Val } from '../../rust';
import { cws } from '../../testHelpers/cws';

expect.extend({
    toContainRust(received: Rust[], prop: any, value: String) {
        if (!received.find((x: any) => x[prop] === value)) {
            return {
                pass: false,
    ...
    toContainUse(received, value) {
        if (getItemsOfType(received.items, 'Use').find((x: any) => x['path'] === value)) {
            return {
                pass: true,
    ...
    toContainAnnotation(received, value) {
        if (getItemsOfType(received.annotations, 'Annotation').find((x: any) => x['value'] === value)) {
            return {
                pass: true,
...
function findItem<T extends Rust>(items: Rust[], prop: string, value: string) {
    return items.find((x: any) => x[prop] === value) as T;
}

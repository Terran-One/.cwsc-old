...
exports.CWScriptScope = exports.Subspace = void 0;
/**
 * Common scope subspaces.
 */
var Subspace;
(function (Subspace) {
    Subspace["TYPE"] = "type"; /** used for struct, enum, type aliases */
    Subspace["FN"] = "fn"; /** used for functions */
    Subspace["STATE"] = "state"; /** used for contract state */
    Subspace["LOCAL"] = "local-var"; /** used for local variables */
    Subspace["ARG"] = "arg"; /** used for function arguments */
    Subspace["ERROR"] = "error"; /** used for error definitions  */
    Subspace["EVENT"] = "event"; /** used for event definitions */
})(Subspace = exports.Subspace || (exports.Subspace = {}));
/**
...
class CWScriptScope {
     ...
     */
    resolve(subspace, identifier) {
        var _a, _b, _c;
        if (subspace === null) {
            // search all subspaces
            for (let ss of this.symbolTable.keys()) {
                let symbol = (_a = this.symbolTable.get(ss)) === null || _a === void 0 ? void 0 : _a.get(identifier);
                if (symbol !== undefined) {
                    return symbol;
                }
            }
            ...
            if (Array.isArray(subspace)) {
                for (let ss of subspace) {
                    let symbol = (_b = this.symbolTable.get(ss)) === null || _b === void 0 ? void 0 : _b.get(identifier);
                    if (symbol !== undefined) {
                        return symbol;
                    }
                }
                ...
                let symbol = (_c = this.symbolTable.get(subspace)) === null || _c === void 0 ? void 0 : _c.get(identifier);
                if (symbol !== undefined) {
                    return symbol;
                }
            }
     ...
     */
    define(subspace, identifier, symbol) {
        if (!this.symbolTable.has(subspace)) {
            // create a new subspace
            this.symbolTable.set(subspace, new Map());
        }
    ...
    applyDefn(defn) {
        for (const subspace in defn) {
            for (const identifier in defn[subspace]) {
                this.define(subspace, identifier, defn[subspace][identifier]);
            }
        }
    }
}
exports.CWScriptScope = CWScriptScope;
